<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Front-End or Web Language Lines
    |--------------------------------------------------------------------------
    |
    |
    */

    //Home
    'developer' => 'Developer',
    'what-i-do' => 'What I do',
    'myservices' => 'My services',
    'services-description' => 'Services description',
    'read-more' => 'Read more',
    'web-sites-manageables' => 'Websites manageables',
    'web-applications' => 'Web applications',
    'other-web-projects' => 'Other web projects',
    'my-web-projects' => 'My web projects',
    'my-portfolio' => 'My portfolio',
    'view-all' => 'View all projects',
    'home' => 'Home',

    
    
    

    //About me
    'working-over-years' => 'Freelance since 2021',
    'my-details' => 'Contact data',
    'experience' => 'Experience',
    'technologies' => 'Technologies',
    'languages' => 'Languages',
    'education' => 'Education',
    'name' => 'Name',
    'city' => 'City',
    'degree-1' => 'Civil Engineer',
    'degree-2' => 'Computer Science Degree',


    //Services
    'categories' => 'Categories',


    //Projects
    'period-time' => 'Project dates',
    'company' => 'Company',
    'location' => 'Location',
    'project-link' => 'Project Url',
    'category' => 'Category',


    //Blog
    'blog' => 'Blog',
    'by-author' => 'Jordi Llobet',
    'post-tag' => 'Post Tags',
    'share-on' => 'Share On',
    'next-post' => 'Next Post',
    'previous-post' => 'Previous Post',
    'related-posts' => 'Related posts',
    'blog-details' => "Blog details",


    //Contact
    'contact-now' => 'Contact now',
    'phone' => 'Phone',
    'email' => 'Email',
    'need-help-contact' => 'Do you need help?, contact me',
    'need-help-contact-text' => 'You can fill in and send the following form. Or, if you prefer, send an email to',
    'your-name' => 'Your name',
    'your-email' => 'Your email',
    'your-phone' => 'Your phone',
    'your-subject' => 'Subject',
    'your-message' => 'Your message',
    'send-message' => 'Send message',
    'accept-privacy-policy' => 'Accept your',

    //Footer
    'lets-chat' => 'Contact',
    'follow-me' => 'Follow me',
    'home' => 'Home',
    'about-me' => 'About me',
    'services' => 'Services',
    'projects' => 'Projects',
    'blog' => 'Posts',
    'contact' => 'Contact',
    'all-rights-reserved' => 'All rights reserved',
    'legal-notice' => 'Legal notice',
    'cookies-policy' => 'Cookies policy',
    'privacy-policy' => 'Privacy policy',
    'developed-by' => 'Developed by',
    'click-to-call' => 'Clic to call'

];